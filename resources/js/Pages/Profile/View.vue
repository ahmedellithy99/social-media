...
if (authUser.id > props.user.id) {
    chat.value = String(props.user.id) + String(authUser.id);
} else {
    chat.value = String(authUser.id) + String(props.user.id);
}

function chating() {
    router.get(route("chat.index", chat.value));
}
</script>
    ...
    >
        ...
        <div class="max-w-[768px] mx-auto h-full overflow-auto px-6">
            <div class="relative bg-white">
                ...
                <div class="group">
                    <img
                        @click="openCoverPhoto"
                        :src="
                            coverImageSrc ||
                            user.cover_url ||
                            '/img/default-cover.jpg'
                        "
                        class="w-full h-[200px] object-cover rounded cursor-pointer z-30"
                    />
                    ...
                    <div v-else class="flex absolute top-2 right-2 gap-3">
                        <button
                            @click="resetCoverImgae"
                            class="bg-gray-50 hover:bg-gray-100 text-gray-800 py-1 px-2 text-xs"
                        >
                            Cancel
                        </button>
                        <button
                ...
                <div class="flex items-center">
                    <!-- Profile Photo -->
                    <div
                        class="flex items-center justify-center relative group/avatar -mt-[64px] ml-[10px] w-[128px] h-[128px] rounded-full"
                    >
                        <img
                    ...
                    <div class="my-auto">
                        <PrimaryButton
                            @click="follow"
                            v-if="!isFollowing && authUser.id != user.id"
                        >
                            Follow
                        </PrimaryButton>
                    </div>
